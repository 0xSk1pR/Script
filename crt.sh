#this if i want give him domain
#!/bin/bash
if [ $# -eq 0 ]; then
...
# Check if a search domain is provided as an argument
if [ $# -eq 0 ]; then
  echo "Usage: $0 <domain-to-filter>"
  exit 1
fi
...
# Use curl to send a GET request to crt.sh and use jq to filter the JSON output
curl -s "https://crt.sh/?q=${search_domain}&output=json" | jq -r --arg domain "$search_domain" '.[] | select(.name_value | contains($domain)) | .name_value'
